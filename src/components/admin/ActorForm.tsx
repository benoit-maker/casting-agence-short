...
import { Upload, X, Link, Film, Play, Plus } from "lucide-react";
    ...
    <form onSubmit={handleSubmit}>
      <Card className="p-6 space-y-6">
        ...
        <div>
          ...
          {videos.length > 0 && (
            <div className="space-y-2 mb-3">
              {videos.map((url, i) => {
                ...
                return (
                  <div key={i} className="flex items-center gap-3 p-2 bg-gray-50 rounded-btn border border-gray-200">
                    {thumb ? (
                      /* eslint-disable-next-line @next/next/no-img-element */
                      <img src={thumb} alt="" className="w-16 h-10 rounded object-cover flex-shrink-0" />
                    ) : (
                      <div className="w-16 h-10 rounded bg-primary-light flex items-center justify-center flex-shrink-0">
                    ...
                    <button
                      ...
                      className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-btn transition-colors cursor-pointer flex-shrink-0"
                      title="Supprimer"
                    >
                      <X className="w-4 h-4" />
                    </button>
          ...
          {/* Bouton + Ajouter */}
          {videos.length < MAX_VIDEOS && !uploadingVideo && (
            <div>
              {addMode === null ? (
                <button
                  type="button"
                  onClick={() => setAddMode("url")}
                  className="flex items-center gap-2 px-3 py-2 rounded-btn border border-dashed border-gray-300 text-sm text-gray-500 hover:border-primary hover:text-primary transition-colors cursor-pointer"
                >
                  <Plus className="w-4 h-4" />
                  Ajouter une vidéo
                </button>
              ) : (
                <div className="border border-gray-200 rounded-card p-4 space-y-3 bg-gray-50">
                  {/* Tabs URL / Upload */}
